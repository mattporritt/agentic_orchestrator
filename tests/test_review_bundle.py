...
def test_generate_review_bundle_can_use_mock_fallback(tmp_path: Path, monkeypatch) -> None:
    ...
    config_used = json.loads((bundle_dir / "config_used.json").read_text(encoding="utf-8"))
    ...
    assert (bundle_dir / "debug_eval.json").exists()
    assert (bundle_dir / "debug_eval.txt").exists()
    assert (bundle_dir / "health.json").exists()
    assert (bundle_dir / "health.txt").exists()
    assert (bundle_dir / "health_warning.txt").exists()
    assert (bundle_dir / "health_failure.txt").exists()
    assert (bundle_dir / "verify.json").exists()
    assert (bundle_dir / "verify.txt").exists()
    assert (bundle_dir / "verify_degraded.txt").exists()
    assert (bundle_dir / "verify_failure.txt").exists()
    assert (bundle_dir / "pilot_report.txt").exists()
    assert (bundle_dir / "pilot_report.json").exists()
    assert (bundle_dir / "mode_comparison.md").exists()
    assert (bundle_dir / "task_eval.txt").exists()
    assert (bundle_dir / "example_outputs" / "thin_result.synthetic.json").exists()
    assert (bundle_dir / "README_snapshot.md").exists()
    assert (bundle_dir / "AGENTS.md").exists()
    assert (bundle_dir / "CONTRIBUTING.md").exists()
    assert (bundle_dir / "docs_checklist.md").exists()
    assert (bundle_dir / "refactor_map.md").exists()
